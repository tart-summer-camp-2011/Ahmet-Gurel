...
class sniffLoginController extends sniffController {
   ...
   public function execute(sniffRequest $request, sniffResponse $response) {
      ...
      if ($request->getValue('sniffUser')) {
         ...
         //setcookie('sniffUser', serialize($_SESSION['user']),time()+60*60*24*31, '/');1 ay süren session time
		 setcookie('sniffUser', serialize($_SESSION['user']),time()+60*60, '/');// session time 
      }
